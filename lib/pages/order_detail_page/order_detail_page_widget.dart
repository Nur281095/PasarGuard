...
class _OrderDetailPageWidgetState extends State<OrderDetailPageWidget>
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.orderDetailResult = await PasargadrugsGroup.orderDetailCall.call(
        token: currentAuthenticationToken,
        id: widget!.id,
      );

      if ((_model.orderDetailResult?.succeeded ?? true)) {
        _model.order = PasargadrugsGroup.orderDetailCall.order(
          (_model.orderDetailResult?.jsonBody ?? ''),
        );
        _model.products = PasargadrugsGroup.orderDetailCall
            .products(
              (_model.orderDetailResult?.jsonBody ?? ''),
            )!
            .toList()
        ...
        _model.shippingAddress =
            PasargadrugsGroup.orderDetailCall.shippingAddress(
          (_model.orderDetailResult?.jsonBody ?? ''),
        );
        _model.billingAddress =
            PasargadrugsGroup.orderDetailCall.billingAddress(
          (_model.orderDetailResult?.jsonBody ?? ''),
        );
        safeSetState(() {});
        return;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              PasargadrugsGroup.orderDetailCall.message(
                (_model.orderDetailResult?.jsonBody ?? ''),
              )!,
              style: TextStyle(
  ...
  @override
  void didUpdateWidget(OrderDetailPageWidget oldWidget) {
                                    ...
                                    Align(
                                      ...
                                      child: Padding(
                                        padding: EdgeInsetsDirectional.fromSTEB(
                                            0.0, 30.0, 0.0, 0.0),
                                        child: Text(
                                          '${_model.products.length.toString()} items',
                                          style: FlutterFlowTheme.of(context)
                                              .bodyMedium
                                              .override(
                                                font: GoogleFonts.inter(
